...
const investmentTypes = [
  { key: 'stock', name: '股票', color: '#3B82F6', description: '高风险高收益，适合长期投资' },
  { key: 'bond', name: '债券', color: '#10B981', description: '低风险稳定收益，适合保守投资者' },
  { key: 'etf', name: 'ETF', color: '#8B5CF6', description: '分散投资，跟踪指数表现' },
  { key: 'mutual-fund', name: '基金', color: '#F59E0B', description: '专业管理，多样化投资' },
  { key: 'crypto', name: '加密货币', color: '#EF4444', description: '极高风险，高波动性' },
  { key: 'real-estate', name: '房地产', color: '#06B6D4', description: '实物资产，抗通胀' },
  { key: 'other', name: '其他', color: '#6B7280', description: '另类投资，如黄金、艺术品等' },
]
...
export default function InvestmentRadarChart({ 
  ...
  useEffect(() => {
    ...
    const data = investmentTypes.map(type => {
      const current = currentAllocation?.find(item => item.type === type.name)?.percentage || 
                     Math.floor(Math.random() * 30) + 5
      const ideal = idealAllocation.find(item => item.type === type.name)?.ideal || 15
        ...
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg max-w-xs">
          ...
          <div className="space-y-1">
            <p className="text-sm text-gray-600">
              当前: <span className="font-medium text-blue-600">{current}%</span>
            </p>
            <p className="text-sm text-gray-600">
            ...
            <p className={`text-sm font-medium ${
              ...
              {difference > 0 ? `超出 ${difference}%` : `不足 ${Math.abs(difference)}%`}
            </p>
          </div>
        </div>
      )
      ...
      <div className="space-y-4">
        <div className="p-4 bg-blue-50 rounded-lg">
          ...
          <div className="space-y-2">
            ...
            {chartData.filter(item => Math.abs(item.current - item.ideal) <= 5).length === chartData.length && (
        ...
        <div className="p-4 bg-green-50 rounded-lg">
          <h4 className="text-sm font-medium text-green-900 mb-2">投资策略建议</h4>
